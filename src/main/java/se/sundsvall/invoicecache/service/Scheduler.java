...
public class Scheduler {
    ...
    private void startFetchingInvoices() throws JobInstanceAlreadyCompleteException, JobExecutionAlreadyRunningException, JobParametersInvalidException, JobRestartException {
        ...
        //If the invoice job ended with a successful exitstatus, and we actually have any invoices, backup the fetched invoices.
        ...
        if(executionResult.getExitStatus().equals(ExitStatus.COMPLETED) && jobHelper.invoiceTableHasInvoices()) {
            ...
            //We cannot know if fetching invoices went ok, since we always delete the whole invoices table beforehand we always need to restore it.
            restoreBackup();
        }
    }
