...
public class InvoiceListener implements StepExecutionListener {
    ...
    public void beforeStep(final StepExecution stepExecution) {
        LOG.info("Before job execution there are {} items in the db", invoiceEntityRepository.count());
        LOG.info("Starting to clean local DB.");
        invoiceEntityRepository.deleteAllInBatch();
        LOG.info("Done cleaning local DB.");
    }
    ...
    public ExitStatus afterStep(final StepExecution stepExecution) {
        ...
        if(stepExecution.getExitStatus().equals(ExitStatus.COMPLETED)) {
            LOG.info("After job execution there are {} items in the db", count);
        } else {
